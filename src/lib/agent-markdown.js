...
function routeMarkdown(path) {
  ...
  if (path === ROUTES.home.path) {
    lines.push(
      "",
      "## Choose by failure state",
      "Futurebits routes work through three isolated practices. Start with the problem, not a catalog.",
      `- [AI & Automation](${SITE_URL}${ROUTES.ai.path}): a repeated workflow is expensive and ready to be tested against real inputs`,
      `- [Markets](${SITE_URL}${ROUTES.markets.path}): research has outrun execution, observability, or risk controls`,
      `- [Design](${SITE_URL}${ROUTES.design.path}): activation, onboarding, upgrade, or conversion has a named point of friction`,
      "",
      "## What delivery produces",
      "- A one-page written scope with acceptance checks and a cut line",
      "- Repository access and a staging path in week one",
      "- Weekly demos against representative inputs",
      "- Code, instrumentation, a runbook, and rollback notes that stay with the client",
      "",
      "## First useful milestone",
      "A focused slice usually runs 2–4 weeks. In the first 2–3 weeks the goal is one bounded workflow, realistic shadow path, or audited product flow that can be checked on staging.",
      "",
      "## Fit",
      "A useful first gate, access to real inputs, and one person able to make scope decisions. Futurebits is not a fit for anonymous staff augmentation, strategy-only work, unattended AI with no owner, capital management, or a standalone logo.",
      "",
      "## UAE & GCC",
